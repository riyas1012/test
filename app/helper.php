...
function sms_notification($id, $type, $message)
{
    ...
    if ($sms_notification == 1) {
        $AccountSid = 'AC5169efa642d1f03850df3b765ca89df2';//Config::get('app.twillo_account_sid');
        $AuthToken = 'f91142844000be90c1289f5976cae8be';//Config::get('app.twillo_auth_token');
        $twillo_number ='+37052140882'; //Config::get('app.twillo_number');
...
function send_notifications($id, $type, $title, $title_lithuanian, $message)
{
    ...
    if ($push_notification == 1) {
        if ($user->device_type == 'ios') {
            ...
            send_ios_push($user->device_token, $title, $title_lithuanian, $message, $type);
        } else {

            $message = json_encode($message);

            send_android_push($user->device_token, $title, $message, $title_lithuanian);
        }
    }
}

function send_ios_push($user_id, $title, $title_lithuanian, $message, $type)
{
    if ($type == 'walker') {
    ...
    $msg = array("alert" => $title,
        "status" => "success",
        "title" => $title,
		"title_lithuanian"=> $title_lithuanian,
        "message" => $message,
        "badge" => 1,
        "sound" => "default");
...
function send_ios_push2($user_id, $title, $title_lithuanian, $message, $type)
{
    if ($type == 'walker') {
        include_once 'ios_push/walker/apns.php';
    } else {
...
function send_android_push($user_id, $message, $title, $title_lithuanian)
{
    include_once 'gcm/GCM_1.php';
    include 'gcm/const.php';
	...
	if (!isset($title_lithuanian) || empty($title_lithuanian)) {
        $title_lithuanian1 = "Message not set";
    } else {
        $title_lithuanian1 = trim($title_lithuanian);
    }

    $message = array(TEAM => $title1, MESSAGE_LITHUANIAN=> $title_lithuanian1, MESSAGE => $msg);
...
return array(
    ...
    */
    ...
    'from' => array('address' => '$email_address.co', 'name' => '$email_name'),
...
function generate_services_config($mandrill_secret){
